...
use verso_symbolic::random_search::{Direction, IndexedRuleSet, RuleDirectionId};
...
pub fn run_policy_evaluation<B: Backend>(config: PolicyEvalConfig, device: B::Device) {
    let rules = IndexedRuleSet::new(verso_symbolic::RuleSet::full());
...
mod tests {
    ...
    fn test_inference_loop_stops_on_no_improvement() {
        ...
        let expr = verso_symbolic::parse_expr("1").unwrap();
        let (result, _trace) =
