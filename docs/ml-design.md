...
| **M4** | Supervised baseline model | Rust: Burn transformer encoder-decoder trained on beam search traces | **Done** — `ogma_training/src/model.rs`, `train.rs` |
| **M5** | Sequence validation harness | Rust: apply predicted action sequences, compute reward via direct function calls | **Done** — `validate.rs`, `ogma_training/src/evaluate.rs` |
| **M6** | Self-play training | Rust: REINFORCE with Burn autograd and direct validation | **Done** — `ogma_training/src/rl_train.rs` |
| **M7** | ~~ONNX inference~~ | Native Rust inference — free with Burn (no export needed) | **Done** |
| **M8** | Production integration | Rust: ML simplify with beam search fallback | **Done** — `ogma_training/src/ml_simplify.rs`, `bin/ml_repl.rs` |
