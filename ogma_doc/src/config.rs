...
pub const SCHEMA_FILENAME: &str = "ogma.schema.json";
pub const SCHEMA_REF: &str = "./ogma.schema.json";
pub const SCHEMA_CONTENT: &str = include_str!("../../schema/v0.1.0/ogma.schema.json");
...
pub struct VersoConfig {
    ...
    /// Version of ogma that last successfully processed this config
    #[serde(default)]
    pub ogma: Option<String>,
    #[serde(default)]
...
/// Generate the default `.ogma.jsonc` content for `ogma init`.
pub fn default_config_content() -> String {
  ...
  // Single paper — specify the main .ogma file
  "input": "paper.ogma"
  ...
  // Test roots — checked by 'ogma check' but not built:
  // "tests": [
  //   {{ "input": "paper.test.ogma" }}
  // ]
}}
"#,
        SCHEMA_REF, VERSION
...
/// Update the `"$schema"` and `"ogma"` fields in the config file,
...
fn stamp_config_text(text: &str) -> String {
    let stamps: &[(&str, &str)] = &[("$schema", SCHEMA_REF), ("ogma", VERSION)];
...
pub const CONFIG_FILENAMES: &[&str] = &[".ogma.jsonc", ".ogma.json"];
pub const CONFIG_FILENAME: &str = ".ogma.jsonc";

/// Search for a config file in the given directory.
/// Checks `.ogma.jsonc` first, then `.ogma.json`.
...
mod tests {
    ...
    fn unique_temp_dir(prefix: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!("ogma-{prefix}-{}-{}", std::process::id(), id))
    }
    ...
    fn parse_single_input() {
        let cfg = VersoConfig::from_jsonc(
            r#"{
            "outputDirectory": "build",
            "input": "src/paper.ogma"
        }"#,
        )
        .unwrap();
        assert_eq!(cfg.output_directory, Some("build".into()));
        assert_eq!(cfg.input, Some("src/paper.ogma".into()));
    ...
    fn resolve_single_input_defaults_output() {
        ...
        assert_eq!(papers[0].input, "src/paper.ogma");
    ...
    fn resolve_papers_defaults_output_to_stem() {
        let cfg = VersoConfig {
            ...
            papers: Some(vec![
                PaperConfig {
                    input: "src/gateway-sw.ogma".into(),
                    output: None,
                },
                PaperConfig {
                    input: "src/main.ogma".into(),
    ...
    fn find_config_prefers_jsonc() {
        ...
        std::fs::write(dir.join(".ogma.jsonc"), "{}").unwrap();
        std::fs::write(dir.join(".ogma.json"), "{}").unwrap();
        let found = find_config(&dir).unwrap();
        assert_eq!(found.file_name().unwrap(), ".ogma.jsonc");
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn find_config_falls_back_to_json() {
        ...
        std::fs::write(dir.join(".ogma.json"), "{}").unwrap();
        let found = find_config(&dir).unwrap();
        assert_eq!(found.file_name().unwrap(), ".ogma.json");
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn load_config_reads_jsonc_file() {
        ...
        assert_eq!(cfg.input, Some("main.ogma".into()));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn default_config_is_valid_jsonc() {
        ...
        assert_eq!(cfg.ogma, Some(VERSION.into()));
        assert_eq!(cfg.output_directory, Some("build".into()));
        assert_eq!(cfg.input, Some("paper.ogma".into()));
    }
    ...
    fn stamp_replaces_existing_fields() {
        ...
        assert!(result.contains(&format!("\"ogma\": \"{}\"", VERSION)));
    ...
    fn resolved_config_inputs() {
        let config = ResolvedConfig {
            ...
            papers: vec![
                ResolvedPaper {
                    input: "a.ogma".to_string(),
                    output: "a".to_string(),
                },
                ResolvedPaper {
                    input: "b.ogma".to_string(),
                    output: "b".to_string(),
                },
            ],
            tests: vec![],
    ...
    fn parse_config_with_tests() {
        ...
        assert_eq!(tests[0].input, "paper.test.ogma");
    }
