...
pub fn find_decl_line(name: &str, text: &str) -> Option<usize> {
    let base = ogma_symbolic::context::subscript_base(name);
        ...
        for prefix in &["var ", "def ", "func "] {
            if let Some(rest) = trimmed.strip_prefix(prefix) {
                let decl_name = rest
                    ...
                    .unwrap_or("")
                    .trim();
                let decl_base = ogma_symbolic::context::subscript_base(decl_name);
...
pub fn find_symbol<'a>(symbols: &'a [SymbolInfo], query: &str) -> Option<&'a SymbolInfo> {
    ...
    let query_base = ogma_symbolic::context::subscript_base(query);
    symbols
        .iter()
        .find(|s| ogma_symbolic::context::subscript_base(&s.name) == query_base)
}
