...
use ogma_symbolic::{subscript_base, Context, Expr, ExprKind};
use ogma_symbolic::rational::Rational;
...
fn verify_claim(claim: &Claim, ctx: &Context) -> VerificationResult {
    ...
    let outcome = match claim.relation {
        ClaimRelation::Eq => match ctx.check_equal(&claim.lhs, &claim.rhs) {
            ogma_symbolic::EqualityResult::Equal => Outcome::Pass,
            ogma_symbolic::EqualityResult::NumericallyEqual { samples, residual } => {
