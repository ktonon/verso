use ogma_symbolic::{Dimension, Expr};

/// A parsed `.ogma` document.
#[derive(Debug)]
pub struct Document {
    pub blocks: Vec<Block>,
}

/// A top-level block in an `.ogma` document.
#[derive(Debug)]
pub enum Block {
...
pub enum ProseFragment {
    /// Plain text.
    Text(String),
    /// Tagged inline math: math`expr` — parsed via ogma_symbolic, rendered via ToTex.
    Math(Expr),
