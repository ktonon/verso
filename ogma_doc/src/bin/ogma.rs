...
#[command(
    name = "ogma",
    version,
    about = "Ogma — verifiable source for scientific papers"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
#[derive(Subcommand)]
enum Command {
    /// Verify claims in .ogma documents.
    /// With no arguments, reads .ogma.jsonc config.
    Check {
        /// .ogma files to check (optional if .ogma.jsonc exists)
        files: Vec<String>,
        /// Watch files and re-run on save
        #[arg(short, long)]
        watch: bool,
    },
    /// Build .ogma documents to PDF or LaTeX.
    /// With no arguments, reads .ogma.jsonc config.
    Build {
        /// .ogma file to build (optional if .ogma.jsonc exists)
        file: Option<String>,
    ...
    Clean,
    /// Initialize a new ogma project (creates .ogma.jsonc)
    Init,
...
fn main() {
    ...
    match cli.command {
        ...
        Command::Repl => {
            if let Err(e) = ogma_symbolic::repl::run() {
...
/// Stamp the config file with the current ogma version and schema URL after a successful run.
fn stamp_config_if_present() {
    use ogma_doc::config::{find_config, stamp_config};
...
fn require_config() -> ogma_doc::config::ResolvedConfig {
    ...
    match resolve_config(&cwd) {
        Ok(Some(cfg)) => cfg,
        Ok(None) => {
            eprintln!("error: no .ogma.jsonc or .ogma.json found");
            eprintln!("run 'ogma init' to create one");
            process::exit(1);
        }
        Err(e) => {
            eprintln!("error: {}", e);
...
fn cmd_init() {
    use ogma_doc::config::{default_config_content, install_schema, CONFIG_FILENAME};
...
fn cmd_clean() {
    ...
    let tmp = std::env::temp_dir().join("ogma-build");
    if tmp.exists() {
...
fn plan_config_builds(
    ...
    if output_override.is_some() && config.papers.len() > 1 {
        return Err(
            "cannot use --output when config resolves to multiple papers; set per-paper outputs in .ogma.jsonc instead"
                .to_string(),
        );
    }
...
impl WatchTask {
    fn new<F: Fn() + 'static>(input: &str, run: F) -> Self {
        use ogma_doc::parse::collect_dependencies;
...
fn watch_and_run(tasks: Vec<WatchTask>) {
    ...
    let dominated_extensions: &[&str] = &["ogma", "bib"];
...
mod tests {
    use super::{
        completion_context_for_line, output_is_tex, plan_config_builds, resolve_single_build_output,
    };
    use std::path::Path;
    use ogma_doc::config::{ResolvedConfig, ResolvedPaper};
    ...
    fn plan_config_builds_uses_per_paper_defaults() {
        let config = resolved_config(
            "build",
            &[("src/alpha.ogma", "alpha"), ("src/beta.ogma", "beta")],
        );
        ...
        assert_eq!(
            builds,
            vec![
                ("src/alpha.ogma".to_string(), "build/alpha.pdf".to_string()),
                ("src/beta.ogma".to_string(), "build/beta.pdf".to_string()),
            ]
        );
    }
    ...
    fn plan_config_builds_allows_output_override_for_single_paper() {
        let config = resolved_config("build", &[("src/paper.ogma", "paper")]);
    ...
    fn resolve_single_build_output_defaults_to_pdf_beside_input() {
        let output = resolve_single_build_output(Path::new("src/paper.ogma"), None);
    ...
    fn resolve_single_build_output_preserves_explicit_output() {
        let output =
            resolve_single_build_output(Path::new("src/paper.ogma"), Some("artifacts/final.tex"));
...
async fn cmd_lsp() {
    ...
    use ogma_doc::verify::{verify_document, Outcome};
    ...
    impl LanguageServer for VersoServer {
        ...
        async fn initialized(&self, _: InitializedParams) {
            self.client
                .log_message(MessageType::INFO, "ogma-lsp initialized")
                .await;
        }
        ...
        async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
            ...
            let items: Vec<CompletionItem> =
                ogma_symbolic::unicode::completions(&ctx.typed_prefix)
                    .into_iter()
                    .map(|(name, ch)| {
