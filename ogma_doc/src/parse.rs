...
use ogma_symbolic::{parse_expr, Dimension};

/// Parse an `.ogma` document from source text.
...
";
    ...
    fn use_imports_declarations_only() {
        ...
        .unwrap();
        std::fs::write(
            dir.join("notation.ogma"),
            "# Notation\n\nvar x [L]\n\ndef c := 3*10^8\n\nSome prose paragraph.\n\nclaim foo\n  x = x",
        )
        .unwrap();
        let doc = parse_document_from_file(&dir.join("main.ogma")).unwrap();
        // Should only get var + def from notation.ogma, then claim from main.ogma
        // No section, prose, or claim from notation.ogma
        ...
        for block in &doc.blocks {
            match block {
                ...
                Block::Claim(c) => {
                    assert_eq!(c.name, "test", "only main.ogma claim should be present");
                    has_claim = true;
                }
        ...
        assert!(has_var, "var from notation.ogma should be imported");
        assert!(has_def, "def from notation.ogma should be imported");
        assert!(has_claim, "claim from main.ogma should be present");
        assert!(!has_section, "section from notation.ogma should NOT be imported");
        assert!(!has_prose, "prose from notation.ogma should NOT be imported");
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn use_imports_descriptions() {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("main.ogma"), "use defs.ogma").unwrap();
        std::fs::write(
            dir.join("defs.ogma"),
            "var v [L T^-1]\n  Velocity.\n\ndef c := 3*10^8\n  Speed of light.",
        )
    ...
    fn use_tracks_dependencies() {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("main.ogma"), "use notation.ogma").unwrap();
        std::fs::write(dir.join("notation.ogma"), "var x [L]").unwrap();
        let deps = collect_dependencies(&dir.join("main.ogma")).unwrap();
    ...
    fn use_missing_file_error() {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("main.ogma"), "use nonexistent.ogma").unwrap();
        let err = parse_document_from_file(&dir.join("main.ogma")).unwrap_err();
        assert!(err.message.contains("nonexistent.ogma"));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn use_empty_path_error() {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("main.ogma"), "use").unwrap();
        let err = parse_document_from_file(&dir.join("main.ogma")).unwrap_err();
    ...
    fn use_imports_func() {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("main.ogma"), "use funcs.ogma").unwrap();
        std::fs::write(dir.join("funcs.ogma"), "func KE(m, v) := (1/2)*m*v^2").unwrap();
        let doc = parse_document_from_file(&dir.join("main.ogma")).unwrap();
        assert_eq!(doc.blocks.len(), 1);
        match &doc.blocks[0] {
    ...
    fn parse_include_basic() {
        ...
        .unwrap();
        std::fs::write(dir.join("sub.ogma"), "Sub content.").unwrap();
        let doc = parse_document_from_file(&dir.join("main.ogma")).unwrap();
    ...
    fn parse_include_circular_error() {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("a.ogma"), "!include b.ogma").unwrap();
        std::fs::write(dir.join("b.ogma"), "!include a.ogma").unwrap();
        let err = parse_document_from_file(&dir.join("a.ogma")).unwrap_err();
    ...
    fn parse_include_missing_file_error() {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("main.ogma"), "!include nonexistent.ogma").unwrap();
        let err = parse_document_from_file(&dir.join("main.ogma")).unwrap_err();
        assert!(err.message.contains("nonexistent.ogma"));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn parse_include_nested() {
        ...
        std::fs::write(dir.join("main.ogma"), "!include sub/a.ogma").unwrap();
        std::fs::write(dir.join("sub/a.ogma"), "!include b.ogma").unwrap();
        std::fs::write(dir.join("sub/b.ogma"), "Nested content.").unwrap();
        let doc = parse_document_from_file(&dir.join("main.ogma")).unwrap();
        match &doc.blocks[0] {
            Block::Prose(fragments) => {
            ...
            Block::Prose(fragments) => {
                ...
                match &fragments[1] {
                    ProseFragment::Math(expr) => match &expr.kind {
                        ogma_symbolic::ExprKind::Var { name, dim, .. } => {
                            assert_eq!(name, "v");
                            assert!(dim.is_some());
                            assert_eq!(
                                dim.as_ref().unwrap(),
                                &ogma_symbolic::Dimension::parse("[L T^-1]").unwrap()
                            );
                        }
            ...
            Block::Prose(fragments) => {
                ...
                match &fragments[1] {
                    ProseFragment::Math(expr) => {
                        assert!(
                            matches!(&expr.kind, ogma_symbolic::ExprKind::Quantity(_, _)),
