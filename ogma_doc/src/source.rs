    ...
    for (i, line) in src.lines().enumerate() {
        ...
        if trimmed.starts_with("!include") {
            ...
            if path_str.is_empty() {
                return Err(ParseDocError {
                    line: i + 1,
                    message: "use requires a file path, e.g. use notation.ogma".into(),
                });
            }
...
/// Collect all file dependencies for a `.ogma` file (the file itself + all includes, recursively).
...
/// Parse an `.ogma` file, resolving `!include` directives.
        ...
        if path_str.is_empty() {
            return Err(ParseDocError {
                ...
                message: if directive == "use" {
                    "use requires a file path, e.g. use notation.ogma".into()
                } else {
