...
use ogma_doc::parse::parse_document;
...
fn basic_algebra_all_pass() {
    let src = load_fixture("basic_algebra.ogma");
...
fn trig_identities_all_pass() {
    let src = load_fixture("trig_identities.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
    for r in &report.results {
        assert!(
...
fn should_fail_detects_wrong_claim() {
    let src = load_fixture("should_fail.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
...
fn proof_chain_passes() {
    let src = load_fixture("proof_chain.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
    for r in &report.results {
        assert!(
...
fn full_document_passes() {
    let src = load_fixture("full_document.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
    for r in &report.results {
        assert!(
...
fn numerical_fallback_passes() {
    let src = load_fixture("numerical_fallback.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
    for r in &report.results {
        assert!(
...
fn dimensional_analysis_passes() {
    let src = load_fixture("dimensional.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
    assert_eq!(report.pass_count(), 3);
    for r in &report.results {
...
fn dimensional_mismatch_detected() {
    let src = load_fixture("dim_fail.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
...
fn unit_annotations_pass_dim_check() {
    let src = load_fixture("units.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
    for r in &report.results {
        assert!(
...
fn syntax_guide_parses_and_verifies() {
    let src = load_fixture("syntax_guide.ogma");
    let doc = parse_document(&src).unwrap();
    let report = verify_document(&doc);
    for r in &report.results {
        assert!(
