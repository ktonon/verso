...
Define and implement the full Ogma markup language — a document format for writing
physics papers with machine-verified mathematics. Ogma borrows from markdown where
...
This section documents the intended Ogma syntax, both what exists today and what
...
An Ogma document is a sequence of blocks separated by blank lines. Blocks are
...
Use `math` backticks for expressions that Ogma can parse, verify, and render:

```
The identity math`sin(x)^2 + cos(x)^2` equals one.
```

The expression is parsed by ogma_symbolic, so it uses expression syntax (not LaTeX).
...
| Ogma source | LaTeX output | Notes |
|------------|-------------|-------|
...
A claim is a named equation that Ogma verifies. Verification checks that
...
- Both sides use ogma_symbolic expression syntax.
...
- Justifications after `;` name a rule. If a named rule exists in `RuleSet::full()`,
  Ogma first checks whether that specific rule transforms the previous step into the
  current one.
...
Declare a variable with its physical dimensions. When present, Ogma checks that both
...
- Each line is a separate expression, parsed by ogma_symbolic.
...
Split a document across multiple `.ogma` files:
...
Documents do not need to specify packages — ogma chooses reasonable defaults.
...
Expressions inside `math` backticks, claims, and proofs use ogma_symbolic syntax,
not LaTeX. Key differences from LaTeX:

| Ogma expression | LaTeX equivalent | Notes |
|---------------|-----------------|-------|
...
- **Not a superset of markdown.** Ogma borrows `#` headings, prose paragraphs,
...
- **Not a general-purpose document format.** Ogma is purpose-built for physics and
...
**Key files:**
- `ogma_doc/src/ast.rs` — add `Bold(Vec<ProseFragment>)`, `Italic(Vec<ProseFragment>)` variants
- `ogma_doc/src/parse.rs` — extend `parse_prose_fragments` to detect `**...**` and `*...*`
...
- `ogma_doc/src/compile_tex.rs` — `Bold` → `\textbf{...}`, `Italic` → `\textit{...}`
- `ogma_doc/src/report.rs` — may need minor update for fragment display
...
**Key files:**
- `ogma_doc/src/ast.rs` — add `Block::List { ordered: bool, items: Vec<ListItem> }` and
  `ListItem { fragments: Vec<ProseFragment>, children: Option<List> }` for nesting
- `ogma_doc/src/parse.rs` — detect lines starting with `- ` or `N. ` at the block level;
...
- `ogma_doc/src/compile_tex.rs` — `itemize`/`enumerate` environments with `\item`
...
**Key files:**
- `ogma_doc/src/ast.rs` — add `Block::MathBlock { exprs: Vec<Expr>, aligned: bool }`
- `ogma_doc/src/parse.rs` — detect ` ```math ` opening fence; collect lines until closing
...
- `ogma_doc/src/compile_tex.rs` — `aligned` → `\begin{align*}`, otherwise `\begin{gather*}`
...
**Key files:**
- `ogma_doc/src/ast.rs` — add `ProseFragment::Cite(Vec<String>)` (multiple keys),
  `Block::Bibliography { path: String }`
- `ogma_doc/src/parse.rs` — add `"cite"` to the tag list in `find_tagged_backtick`;
  parse comma-separated keys; detect `!bibliography` directive
- `ogma_doc/src/compile_tex.rs` — `Cite` → `\cite{key1,key2}`;
...
**Key files:**
- `ogma_doc/src/ast.rs` — add `Block::Environment { kind: EnvKind, title: Option<String>, body: Vec<ProseFragment> }`
  and `EnvKind` enum
- `ogma_doc/src/parse.rs` — detect `!theorem`, `!definition`, etc. at block level;
  title is the rest of the directive line; body is indented continuation lines
  (parsed for inline fragments)
- `ogma_doc/src/compile_tex.rs` — emit `\newtheorem` declarations in preamble (once per
...
**Key files:**
- `ogma_doc/src/ast.rs` — add `Block::BlockQuote(Vec<ProseFragment>)`,
  `ProseFragment::Footnote(Vec<ProseFragment>)`
- `ogma_doc/src/parse.rs`:
...
- `ogma_doc/src/compile_tex.rs` — `BlockQuote` → `\begin{quote}...\end{quote}`,
  `Footnote` → `\footnote{...}`
...
```bash
npm run check -- file.ogma           # verify claims and dimensions
npm run compile -- file.ogma         # compile to LaTeX
npm run watch -- file.ogma           # re-verify on save
